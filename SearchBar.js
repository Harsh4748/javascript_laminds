...
function showusers(users) {
    users.forEach((data) => {
        ...
        //div(card)--> img(img)--> div(blur)->(div(content))
...
inp.addEventListener("input", () => {
    ...
    let new_user = users.filter((data) => {
        ...
        return data.name.startsWith(text) //data mate true aave te dta return(save)thase

        //search bar --> inp.value.toLowerCase == user.name.tolowercase

        //method:inp.value.toChar(0).toUppercase + inp.value.slice(1)
    })
